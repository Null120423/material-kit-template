...
export const daysLeftToExpire = (date: string | Date): number => {
  const today = moment();
...
interface HelperType {
  getGroupDateTime: (date: Date | string, time: Date) => Date;
  getKeyExtractor: (item: any, index: number) => string;
  replaceText: (text: string) => string;
  formatVND: (money: number, prefix?: string) => string;
  formatVNDNumber: (money: number) => string;
  formatDate: (text: string | Date, format?: string) => string;
...
const Helper: HelperType = {
  ...
  getKeyExtractor: (item: any, index: number): string => `${item}-${index}`,

  replaceText: (text: string): string => text.replace(/[\w\d]+/g, '******'),

  formatVND: (money: number, prefix: string = 'VNĐ'): string =>
